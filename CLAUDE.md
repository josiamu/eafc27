...
Bilingual (th default, en) static site teaching EA FC 27 skill moves and controls. Plan and progress: `PLAN.md`.
...
- **Content lives in JSON**, never in components. Moves: `src/data/moves/<slug>.json`, validated by `src/data/schema.ts` in `src/data/moves.ts`. Controls: `src/data/controls/<group>.json`, validated by `src/data/control-schema.ts`. Invalid data must fail the build.
...
- A move or control may be `"verified": true` only when at least two sources agree on stars and inputs. Otherwise `false`, and the site shows a "needs checking" badge.
